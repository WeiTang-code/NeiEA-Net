...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            if dataset.name == 'SensatUrban':
                self.Log_file = open('log_train_' + dataset.name + self.time_stamp + '.txt', 'a')
        with tf.variable_scope('layers'):
            self.logits, self.xyz = self.inference(self.inputs, self.is_training)
    ...
    def inference(self, inputs, is_training):
        ...
        for i in range(self.config.num_layers):
            ...
            input_neigh_idx_new = tf.py_func(DP.knn_search, [input_xyz, input_xyz,19], tf.int32)
            input_neigh_idx = input_neigh_idx_new[:, :, :16]
            n_pts = n_pts // ratio[i]
            sub_xyz, inputs_sub_idx = tf.cond(tf.equal(is_training, tf.constant(True)),
                                              lambda: sampling(self.config.batch_size, n_pts, input_xyz,
                                                               input_neigh_idx),
            ...
            f_encoder_i = self.nei_ea(feature, input_xyz, input_neigh_idx_new, d_out[i],
        ...
        for n in range(self.config.num_layers):
            ...
            for j in range(self.config.num_layers - n):
                ...
                f_decoder_i = helper_tf_util.conv2d_transpose(
                    tf.concat([f_encoder_list[-j - 2 - n], f_interp_i], axis=3),
            ...
            curr_weight = helper_tf_util.conv2d(f_decoder_list[-1], 1, [1, 1], 'Decoder_weight_' + str(n), [1, 1],
                                                'VALID', bn=False, activation_fn=None)
        ...
        for i in range(len(f_multi_decoder)):
            f_decoder_final = f_decoder_final + tf.tile(tf.expand_dims(f_weights[:, :, :, i], axis=-1),
    ...
    def nei_ea(self, feature, xyz, neigh_idx, d_out, name, is_training):
        """
        Inputs: 
        ...
        neigh_feat, drop_idx, drop_num = self.neigh_discarding(neigh_feat, feature, 3)
        neigh_xyz = self.gather_neighbour(xyz, neigh_idx)  # B, N, k, 3
        neigh_xyz = self.idx_discarding(neigh_xyz, xyz, drop_idx, drop_num)
        tile_feat = tf.tile(feature, [1, 1, self.k_n, 1])  # B, N, k, d_out/2
        tile_xyz = tf.tile(tf.expand_dims(xyz, axis=2), [1, 1, self.k_n, 1])  # B, N, k, 3

        feat_info = tf.concat([neigh_feat - tile_feat, tile_feat, neigh_feat], axis=-1)  # B, N, k, d_out
        xyz_info = tf.concat([neigh_xyz - tile_xyz, tile_xyz, neigh_xyz], axis=-1)  # B, N, k, 9
        xyz_encoding = helper_tf_util.conv2d(xyz_info, d_out // 2, [1, 1], name + 'mlp7', [1, 1], 'VALID', True,
                                             is_training)  # B, N, k, d_out/2
        feat_encoding = helper_tf_util.conv2d(feat_info, d_out // 2, [1, 1], name + 'mlp8', [1, 1], 'VALID', True,
                                              is_training)  # B, N, k, d_out/2

        # Mixed Local Aggregation
        overall_info = tf.concat([xyz_encoding, feat_encoding],axis=-1)  # B, N, k, d_out
        k_weights = helper_tf_util.conv2d(overall_info, overall_info.get_shape()[-1].value, [1, 1], name + 'mlp9',
                                          [1, 1], 'VALID', bn=False, activation_fn=None)  # B, N, k, d_out
        ...
        overall_encoding = self.weighted_fusion_pooling([overall_info_max, overall_info_weighted_sum])  # B, N, 1, 2*d_out

        # Output Encoding
        overall_encoding = helper_tf_util.conv2d(overall_encoding, d_out * 2, [1, 1], name + 'mlp10', [1, 1], 'VALID', True,
                                                 is_training)  # B, N, 1, d_out
        # output_feat = helper_tf_util.conv2d(overall_encoding, d_out * 2, [1, 1], name + 'mlp11', [1, 1], 'VALID', True,
        #                                     is_training, activation_fn=tf.nn.leaky_relu)  # B, N, 1, 2*d_out
        shortcut = helper_tf_util.conv2d(feature, d_out * 2, [1, 1], name + 'shortcut', [1, 1], 'VALID',
                                         activation_fn=None, bn=True, is_training=is_training)
        output_feat = tf.nn.leaky_relu(overall_encoding+shortcut)
        return output_feat
    ...
    @staticmethod
    def neigh_discarding(feat_nei, feat_orig, drop_num):
        # feat_nei->(b,n,k,c)

        batch_size = tf.shape(feat_nei)[0]
        num_points = tf.shape(feat_nei)[1]
        num_neigh = tf.shape(feat_nei)[2]
        c = feat_nei.get_shape()[3].value
        # k = 18
        x = tf.reshape(feat_nei, shape=[-1, num_neigh, c])
        orig_x = x
        x = tf.square(x - tf.tile(tf.reshape(feat_orig, shape=[-1, 1, c]), (1, num_neigh, 1)))
        x = tf.reduce_sum(x, axis=-1, keepdims=True)
        # i = tf.argmax(x, axis=-2)
        idx = tf.squeeze(tf.argsort(x, axis=-2), axis=-1)
        idx = idx[:, :-int(drop_num)]
        temp = tf.batch_gather(orig_x, idx)
        new_x = tf.reshape(temp, [batch_size, num_points, num_neigh - drop_num, c])

        return new_x, idx, drop_num

    @staticmethod
    def idx_discarding(feat_nei, feat_orig, drop_idx, drop_num):
        # feat_nei->(b,n,k,c)
        ...
        num_neigh = tf.shape(feat_nei)[2]
        c = feat_nei.get_shape()[3].value
        orig_x = tf.reshape(feat_nei, shape=[-1, num_neigh, c])
        temp = tf.batch_gather(orig_x, drop_idx)
        new_x = tf.reshape(temp, [batch_size, num_points, num_neigh - drop_num, c])

        return new_x

    @staticmethod
    def weighted_fusion_pooling(list_):
        volume = []
        for list_i in list_:
            batch_size = tf.cast(tf.shape(list_i)[0],dtype=tf.float32)
            num_points = tf.cast(tf.shape(list_i)[1],dtype=tf.float32)
            num_neigh = tf.cast(tf.shape(list_i)[2],dtype=tf.float32)
            num_c = tf.cast(tf.shape(list_i)[3],dtype=tf.float32)
            i_sum = tf.reduce_sum(tf.reduce_sum(tf.reduce_sum(list_i, axis=-1), axis=-1))
            i_mean = i_sum / (batch_size * num_points * num_neigh * num_c)
            volume.append(i_mean)
        new_list = []
        weighted_list = [0.7,0.3]
        for j in range(len(list_)):
            temp_tf = list_[j] / volume[j]
            temp_tf = temp_tf * weighted_list[j]
            new_list.append(temp_tf)
        new_concat = tf.concat(new_list, axis=-1)
        return new_concat